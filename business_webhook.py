...
def download_and_save_photo(file_id, token):
    """Скачивает фото на сервер и возвращает путь к файлу"""
    try:
        ...
        # Сохраняем файл локально (в папку static)
...
BASE_URL = "https://eyes-see-bot.onrender.com"  # <- ДОБАВЛЕНО ЗДЕСЬ
DATABASE_URL = os.getenv("DATABASE_URL")
BOT_USERNAME = "EyesSeeBot"  # без @
CONNECT_PHOTO_URL = f"{BASE_URL}/static/connect_bot.jpg"  # <- ИЗМЕНЕНО ЗДЕСЬ
        ...
        for filename in os.listdir(media_dir):
            if filename.startswith(token):
                ...
                if os.path.exists(file_path):
                    media_url = f"{BASE_URL}/static/media/{filename}"  # <- ИЗМЕНЕНО ЗДЕСЬ
                    ...
                    try:
                        if msg_type == "photo":
                            tg("sendPhoto", {
                                "chat_id": chat_id,
                                "photo": media_url,
                                "reply_markup": hide  # Кнопка Скрыть ПРЯМО В МЕДИА
                            })
...
def trial_expired_markup(ref_link: str):
    ...
    return {
        "inline_keyboard": [
            ...
            [
                {"text": "⭐ Оплатить звёздами", "callback_data": "pay_stars_1m"}
            ],
            [
...
def webhook():
    ...
    if "business_message" in data:
        ...
        #2.1 Если владелец отвечает на сообщение с исчезающим контентом
        if sender.get("id") == owner_id and "reply_to_message" in msg:
            ...
            # В обработке business_message, когда перехватываем исчезающее фото:
            ...
            # Сохраняем медиа на сервер, иначе бот не сможет его отправить
            download_and_save_media(file_id, token, msg_type)
            ...
            # Отправляем БЕЗ кнопки "Скрыть" - ИСПРАВЛЕНИЕ 1
            send_text(owner_id, header + body + who, markup=None)
    ...
    if "message" in data:
        ...
        if text.startswith("/start"):
                    ...
                    send_photo(
                        ...
                        ),
                        {
                            "inline_keyboard": [[
                                {
                                  "text": "📋 Скопировать",
                                  "web_app": { "url": f"{BASE_URL}/static/copy.html" }  # <- ИЗМЕНЕНО ЗДЕСЬ
                                }
                            ]]
                        }
                    )
            ...
            if re.fullmatch(r"[0-9a-f]{10}", payload):
                ...
                # ИСПРАВЛЕНИЕ 2: Отправляем медиа С КНОПКОЙ "Скрыть" прямо в медиа
                        ...
                        "inline_keyboard": [[
                            {
                                "text": "📋 Скопировать",
                                "web_app": {
                                    "url": f"{BASE_URL}/static/copy.html"  # <- ИЗМЕНЕНО ЗДЕСЬ
                                }
                            }
                        ]]
                    }
            ...
            kb2 = {
                "inline_keyboard": [
                    [{
                        ...
                        "web_app": { "url": f"{BASE_URL}/webapp?mode=restore&chat_id={biz_chat_id}&v=2" }  # <- ИЗМЕНЕНО ЗДЕСЬ
                    }],
