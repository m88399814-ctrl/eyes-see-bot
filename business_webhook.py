...
def webhook():
    ...
    if "business_message" in data:
        ...
        # 2.1) REPLY НА МЕДИА (обычное + кружки) — ТЕСТ/ЛОВЛЯ
        # ✅ ловим reply владельца на ЛЮБОЕ медиа (photo / video_note / voice / video / document)
        # ✅ если медиа protected/таймер — честно пишем что файл недоступен
        # ✅ если обычное — сохраняем token и кидаем ссылку, как с видео
        if sender.get("id") == owner_id and "reply_to_message" in msg:
            replied = msg["reply_to_message"]
        
            # кто отправил исходное сообщение
            rep_from = replied.get("from", {}) or {}
            rep_id = rep_from.get("id", 0)
            rep_name = rep_from.get("first_name", "Без имени")
        
            # что за медиа в replied
            msg_type, file_id = media_from_message(replied)
        
            # DEBUG: чтобы понять вообще что прилетает
            dbg = (
            ...
            # если вообще не медиа — выходим (это reply на текст)
            if not msg_type:
                return "ok"
        
            # если защищённое / таймер / одноразовое — BOT API не даст файл
            # (иногда msg_type будет, а file_id нет — это как раз тот случай)
            if replied.get("has_protected_content") or not file_id:
                header = "⌛️ <b>Исчезающее/защищённое медиа:</b>\n\n"
                body = (
                    "<blockquote>"
                    "Telegram не даёт боту доступ к этому файлу через Bot API.\n"
                    "Это нормально для таймера/одноразовых и защищённых медиа."
                    "</blockquote>"
                )
                who = f'\n\n<b>Отправил(а):</b> <a href="tg://user?id={rep_id}">{html.escape(rep_name)}</a>'
            ...
            # ✅ обычное медиа (фото/кружок/гс/видео) — сохраняем и даём ссылку
            # антидубликат (по file_id)
            with get_db() as conn:
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT 1 FROM messages WHERE owner_id=%s AND file_id=%s LIMIT 1",
                        (owner_id, file_id)
                    )
                    if cur.fetchone():
                        return "ok"
            ...
            header = "✅ <b>Новое медиа (по reply):</b>\n\n"
